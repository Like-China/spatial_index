...
import rtree.rtree_IO.IO.MemoryPageFile;
import rtree.rtree_IO.IO.PageFile;
import rtree.rtree_IO.utils.BranchList;
import rtree.rtree_IO.utils.BranchListMinDistComparator;
import rtree.rtree_IO.utils.Data;
...
public class RTree {
	 ...
	 * @return
	 */
	public List<Data> nearestNeighborSearch(RTNode node, Point queryPoint,
		...
		if (node.isLeaf())// node为叶子结点则比较Point到每个条目的最小距离
		{
			...
			for (int i = 0; i < node.usedSpace; i++) {
				...
				branchList[i] = new BranchList(rtNode, rtNode
						.getNodeRectangle().getMinDist(queryPoint),
						rtNode
