...
import rtree.rtree_IO.Constants;
...
public abstract class RTNode implements INode {
	...
	public RTNode(RTree tree, int parent, int pageNumber, int level) {
		this.parent = parent;
		this.tree = tree;
	...
	/**
	 * @return Returns a unique id for this node. The page number is unique for
	 *         every node.
	 */
	@Override
	...
	public void addData(Rectangle rectangle, int page) {
	 ...
	 *             存储删除结点中剩余条目
	 */
	public void condenseTree(List<RTNode> list) {
		if (isRoot()) {
	 ...
	 * @return 两个组中的条目的索引
	 */
	public int[][] quadraticSplit(Rectangle rectangle, int page) {
		if (rectangle == null) {
	 ...
	 * @return RTDataNode
	 */
	public abstract RTDataNode chooseLeaf(Rectangle rectangle);
