...
public class Octree<T> {
    ...
    public class Oct {
        ...
        public void findAll(double X1, double Y1, double Z1, double X2, double Y2, double Z2, List<CoordHolder> ret) {
            ...
            if (overlap(ULN, X1, Y1, Z1, X2, Y2, Z2))
                ULN.findAll(X1, Y1, Z1, X2, Y2, Z2, ret);
            if (overlap(ULF, X1, Y1, Z1, X2, Y2, Z2))
                ULF.findAll(X1, Y1, Z1, X2, Y2, Z2, ret);
            if (overlap(URN, X1, Y1, Z1, X2, Y2, Z2))
                URN.findAll(X1, Y1, Z1, X2, Y2, Z2, ret);
            if (overlap(URF, X1, Y1, Z1, X2, Y2, Z2))
                URF.findAll(X1, Y1, Z1, X2, Y2, Z2, ret);
            if (overlap(LLN, X1, Y1, Z1, X2, Y2, Z2))
                LLN.findAll(X1, Y1, Z1, X2, Y2, Z2, ret);
            if (overlap(LLF, X1, Y1, Z1, X2, Y2, Z2))
                LLF.findAll(X1, Y1, Z1, X2, Y2, Z2, ret);
            if (overlap(LRN, X1, Y1, Z1, X2, Y2, Z2))
                LRN.findAll(X1, Y1, Z1, X2, Y2, Z2, ret);
            if (overlap(LRF, X1, Y1, Z1, X2, Y2, Z2))
                LRF.findAll(X1, Y1, Z1, X2, Y2, Z2, ret);
        }
        ...
        private boolean overlap(Oct q, double X1, double Y1, double Z1, double X2, double Y2, double Z2) {
            if (q.x2 < X1 || q.y2 < Y1 || q.z2 < Z1 || q.x1 > X2 || q.y1 > Y2 || q.z1 > Z2)
                return false;
            return true;
        }
    ...
    public List<CoordHolder> findAll(double x, double y, double z, double radius) {
        ...
        if (list instanceof LinkedList) {
            ...
            while (it.hasNext()) {
                ...
                double dist = Math.pow(holder.x - x, 2)
